...
BaÄlantÄ±lar: [[00-Home]] Â· [[TODO]] Â· [[CHANGELOG]] Â· [[01_sirket_master_ana_belgesi]]
...
- Company Master V1.0 Åema tasarÄ±mÄ± (19 tablo)

- ETL mimarisi ve veri akÄ±ÅÄ± tanÄ±mlandÄ±

- MVP gereksinimleri (P0/P1/P2) belirlendi

- V9 ile karÅÄ±laÅtÄ±rma yapÄ±ldÄ±

- Ajan ekosistemi tanÄ±mlandÄ±

- Veri kalitesi / sentetik veri / PII tarama prototipi `src/data_quality_toolkit/` altÄ±na taÅÄ±ndÄ±.

- `src/company_master/` paket iskeleti oluÅturuldu (Åema, ETL, entity_resolution, search, services).

- 16 temel test eklendi ve geÃ§ti.

- PostgreSQL migration seti canlÄ± Supabase sunucusuna uygulandÄ±: 4 migration (0001â0004), 22 tablo eksiksiz oluÅturuldu.
- PostgreSQL 17 `pg_dump` kurulumu yapÄ±ldÄ±; `scripts/supabase_backup.py` PATH fallback mekanizmasÄ± ile gÃ¼ncellendi ve ilk sÄ±kÄ±ÅtÄ±rÄ±lmÄ±Å tam veritabanÄ± yedeÄi alÄ±ndÄ± (`backups/supabase_20260902_001559.sql.gz`).
...
- Normalizasyon + arama motoru tamamlandÄ±: `0005` migration (`companies.source_record_id` FK) + `etl/normalize.py` ile 8.313 firma `companies` tablosuna aktarÄ±ldÄ± (idempotent). `search/engine.py` pg_trgm ILIKE arama motoru; test sorgularÄ± Ã§alÄ±ÅtÄ±.

- Entity resolution entegrasyon tamamlandÄ±: `etl/entity_resolution.py` VKN/fuzzy matching ile `source_records` â `companies` eÅleÅtirme, `entity_resolution` tablosuna sonuÃ§ yazar.
...
- Veri toplama iÃ§in fiili kaynak eriÅimi (GÄ°B, Oda, diÄer OSB'ler) baÅlatÄ±lmadÄ±.
...
- MVP sÄ±nÄ±rÄ±: Veri tabanÄ± kurulumunun, temel veri temizliÄinin ve iÅ akÄ±ÅÄ±nÄ±n doÄrulanmadan mÃ¼Återi odaklÄ± web arayÃ¼zÃ¼ne geÃ§ilmez. Ä°lk etapta iÃ§ doÄrulama iÃ§in Streamlit gibi sade ara yÃ¼z kullanÄ±lÄ±r; web aktarÄ±mÄ± veri deÄeri kanÄ±tlandÄ±ktan sonra yapÄ±lÄ±r.
...
- Kod/belge uyumsuzluÄu teknik borÃ§ yaratÄ±yor.
...
### Kod / Belge UyumsuzluÄu (2026-09-01)



- **Karar:** SeÃ§enek A â AyrÄ± ModÃ¼l

- **Uygulama:** `src/` modÃ¼lleri `src/data_quality_toolkit/` altÄ±na taÅÄ±ndÄ±. Company Master iÃ§in `src/company_master/` paket iskeleti oluÅturuldu. data_quality_toolkit artÄ±k ETL boru hattÄ±nda yardÄ±mcÄ± bileÅen olarak kullanÄ±lacak.
...
- Streamlit arayÃ¼zÃ¼ne baÄlandÄ±: http://localhost:8501
...
- Test mesajÄ± baÅarÄ±yla gÃ¶nderildi (message_id: 5)

- Polling baÅlatÄ±ldÄ±, `/status`, `/gorev`, `/rapor` komutlarÄ± Ã§alÄ±ÅÄ±yor
