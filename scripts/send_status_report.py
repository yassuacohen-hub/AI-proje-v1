#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""Son durum raporunu Telegram'a gönder."""
import os
import sys
...
active_tasks = [t for t in board if t.get("durum") == "aktif"]
completed_tasks = [t for t in board if t.get("durum") == "done"]

# Telegram mesajı
lines = [
    "📊 <b>Company Master — Son Durum Raporu</b>",
    "",
    f"🏢 <b>Toplam Firma:</b> {kpi.get('total', 0):,}",
    f"⭐ <b>Kalite Skoru:</b> {kpi.get('avg_score', 0):.1f}/100",
    "",
    ...
    f"  • Telefon: {kpi.get('tel', 0):,} ({kpi.get('tel', 0)/max(kpi.get('total',1),1)*100:.1f}%)",
    ...
    "",
    f"📋 <b>Görev Durumu:</b>",
    f"  • Aktif: {len(active_tasks)}",
    f"  • Tamamlanan: {len(completed_tasks)}",
    "",
    f"🖥️ <b>Dashboard:</b> http://localhost:8501",
    f"🕐 {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}",
]
text_msg = "\n".join(lines)

result = send_message(text_msg)
print("Telegram mesajı gönderildi:", result.get("ok", False))
if not result.get("ok"):
    print("Hata:", result.get("error", "Bilinmeyen hata"))
