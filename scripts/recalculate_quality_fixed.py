#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""DB kalite skorlarini yeniden hesapla - NACE ve diger alanlari kontrol et."""
import sys
            ...
            WITH batch AS (
                SELECT c.company_id,
                    (
                        ...
                        + CASE WHEN COALESCE(sr.raw_address, sr.raw_payload->>'adres') IS NOT NULL
                                    AND COALESCE(sr.raw_address, sr.raw_payload->>'adres') <> '' THEN 20 ELSE 0 END
                        + CASE WHEN COALESCE(c.nace_name, sr.raw_payload->>'sektor') IS NOT NULL
                                    AND COALESCE(c.nace_name, sr.raw_payload->>'sektor') <> '' THEN 10 ELSE 0 END
                        ...
                        + CASE WHEN COALESCE(c.nace_code, sr.raw_nace, sr.raw_payload->>'nace_code') IS NOT NULL
                                    AND COALESCE(c.nace_code, sr.raw_nace, sr.raw_payload->>'nace_code') <> '' THEN 5 ELSE 0 END
                    ) AS new_score
                FROM companies c
