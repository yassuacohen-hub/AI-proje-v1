...
NACE_TAXONOMY_FILE: Path = DATA_DIR / "nace" / "turkiye_nace.json"
...
def _extract_keywords(name_tr: str) -> list[str]:
    """NACE isimden otomatik anahtar kelime cikar."""
    if not name_tr:
        return []
    name_lower = name_tr.lower()
    tr_map = {
    ...
    code_part = name_tr.split()[0] if name_tr.split() else ""
    if '.' in code_part:
        code_nums = code_part.split('.')
...
def load_nace_taxonomy() -> list[NaceRecord]:
    ...
    return [
        ...
        NaceRecord(
            ...
            keywords=row.get("keywords") or _extract_keywords(row.get("name_tr", "")),
...
@lru_cache(maxsize=1)
def _keyword_index() -> tuple[dict[str, tuple[NaceRecord, ...]], tuple[tuple[str, tuple[NaceRecord, ...]], ...]]:
    """NACE taxonomy için kelime-bazlı arama indeksi (performans).

    Tek kelimelik keyword'ler -> dict lookup (O(1)),
    çok kelimelik phrase'ler -> substring kontrol listesi.

    Returns:
        (word_index, phrase_list) ikilisi.
    """
    word_index: dict[str, list[NaceRecord]] = {}
    phrase_list: list[tuple[str, list[NaceRecord]]] = []
    for record in load_nace_taxonomy():
        for kw in record.keywords:
            kw_norm = _normalize_tr(kw)
            if not kw_norm:
                continue
            if " " in kw_norm:
                phrase_list.append((kw_norm, record))
            else:
                word_index.setdefault(kw_norm, []).append(record)
    return (
        {k: tuple(v) for k, v in word_index.items()},
        tuple((p, (r,)) for p, r in phrase_list),
    )
...
def nace_bul(
    ...
    if ostim_sektor and ostim_sektor in ostim_map:
        ...
        word_index, phrase_list = _keyword_index()

        seen: set[int] = set()

        # Cok kelimelik phrase'ler: substring kontrol
        for kw_norm, records in phrase_list:

            if kw_norm in unvan_norm:
        ...
        # Tek kelimelik keyword'ler: unvan kelimeleri ile dict lookup
        for word in set(re.findall(r"\w+", unvan_norm)):

            for record in word_index.get(word, ()):
